...
func TestHandlerUpdateValidRequest(t *testing.T) {
	d := device.Device{
		SerialNum: "645",
		Model:     "a",
		IP:        "1.6.3.76",
	...
	serviceMock.On("UpdateDevice", mock.Anything).Return(nil)
	...
	req, err := http.NewRequest(http.MethodPatch, "/devices", bytes.NewReader(input))
...
func TestHandlerUpdateNotExistingDevice(t *testing.T) {
	d := device.Device{
		SerialNum: "645",
		Model:     "a",
		IP:        "1.6.3.76",
	...
	serviceMock := mocks.NewService(t)
	serviceMock.On("UpdateDevice", d).Return(fmt.Errorf("device not found: %s", d.SerialNum))
	...
	require.Equal(t, fmt.Sprintf("device not found: %s", d.SerialNum), resp.Message)
}
